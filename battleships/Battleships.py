        ...
        while True:
            ...
            try:
                ...
                if (x >= 0 and x < self.__size and y >= 0 and y <self.__size) :
                    return (x,y)
                print(f"Please enter the row and the column coordinate between 0 and {self.__size-1}")

            except:
                print(f"Please enter the row and the column coordinate as a number between 0 and {self.__size-1}")

    def __updatePlayBoard(self, x:int, y:int, info:int):

        self.__playBoard[x][y] = info

    # view, possible to seprate into another file to make it more modular
